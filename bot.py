...
async def final_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text == "✅ Так":
        ...
        try:
            if not ps:
                msg = await context.bot.send_message(CHANNEL_ID, cap, parse_mode=ParseMode.HTML)
            elif len(ps) == 1:
                msg = await context.bot.send_photo(CHANNEL_ID, ps[0], caption=cap, parse_mode=ParseMode.HTML)
            else:
            ...
            await update.message.reply_text(f"❌ Помилка прав в каналі: {e}")
    return ConversationHandler.END
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d[0] == "del":
        ...
        await q.edit_message_text("🗑 Видалено всюди.")
    elif d[0] == "price":
