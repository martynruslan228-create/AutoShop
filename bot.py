...
async def get_desc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Надішліть фото (до 10 шт) і натисніть /done:", reply_markup=ReplyKeyboardMarkup([["➡️ Без фото"]], resize_keyboard=True)); return PHOTOS
...
async def get_tg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"ПЕРЕВІРКА:\n\n{res}\n\nПублікуємо?", reply_markup=ReplyKeyboardMarkup([["✅ Так","❌ Ні"]], resize_keyboard=True), parse_mode=ParseMode.HTML)
    return CONFIRM
...
async def main():
    ...
    # Запуск polling (правильный способ для v20+)
