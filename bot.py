...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "Я допоможу вам опублікувати ваше оголошення на канал.\n"
...
async def show_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not ads:
        ...
        return ConversationHandler.END
    buttons = [[f"ID:{ad[0]} | {ad[1]} {ad[2]} (${ad[3]})"] for ad in ads]
    buttons.append(["❌ Скасувати"])
    txt = "Оберіть авто для зміни ціни:" if mode == "edit" else "Оберіть авто для видалення:"
    await update.message.reply_text(txt, reply_markup=ReplyKeyboardMarkup(buttons, resize_keyboard=True))
    return CHOOSE_CAR
    ...
    try:
        ...
        if res:
            ...
            if context.user_data['mode'] == "del":
                ...
                await update.message.reply_text("💰 Введіть НОВУ ЦІНУ ($):", reply_markup=ReplyKeyboardRemove())
...
async def get_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [["✅ Готово"], ["⏩ Пропустити"]]
    await update.message.reply_text("10. Надішліть фото (максимум 10). Після завершення натисніть «✅ Готово»:", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))
    return PHOTO
...
async def get_city(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [["✅ Так", "❌ Ні"]]
    await update.message.reply_text("13. Показати ваш Telegram для зв'язку?", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))
    return TG_CONTACT
