    ...
    try:
        ...
        if res:
            ...
            if context.user_data['mode'] == "del":
                ...
                await update.message.reply_text("🗑 Видалено успішно!")
                return await start(update, context)
            else:
                await update.message.reply_text(f"💰 Введіть НОВУ ЦІНУ ($) для {choice.split('|')[1]}:", reply_markup=ReplyKeyboardRemove())
...
async def get_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("9. Фото (надішліть та натисніть «Готово»):", reply_markup=ReplyKeyboardMarkup([["✅ Готово"], ["⏩ Пропустити"]], resize_keyboard=True))
    return PHOTO
async def get_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text in ["✅ Готово", "⏩ Пропустити"]:
        kb = [["Одеський", "Ізмаїльський"], ["Подольський", "Болградський"]]
        await update.message.reply_text("10. Район:", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True)); return DISTRICT
    if update.message.photo: context.user_data['photos'].append(update.message.photo[-1].file_id)
    return PHOTO
async def get_district(update: Update, context: ContextTypes.DEFAULT_TYPE): context.user_data['district'] = update.message.text; await update.message.reply_text("11. Місто:"); return CITY
async def get_city(update: Update, context: ContextTypes.DEFAULT_TYPE): context.user_data['city'] = update.message.text; await update.message.reply_text("12. Показати ваш ТГ?", reply_markup=ReplyKeyboardMarkup([["✅ Так", "❌ Ні"]], resize_keyboard=True)); return TG_CONTACT
