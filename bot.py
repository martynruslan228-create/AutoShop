...
# --- Состояния ---
(MAKE, MODEL, YEAR, GEARBOX, FUEL, DRIVE, DISTRICT, TOWN, PRICE, DESCRIPTION, PHOTOS, PHONE, SHOW_CONTACT, CONFIRM, EDIT_PRICE) = range(15)
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "del":
        try:
            await context.bot.delete_message(CHANNEL_ID, int(data[2]))
        except:
            pass
        conn = sqlite3.connect(DB_PATH)
        conn.execute('DELETE FROM ads WHERE id = ?', (db_id,))
        conn.commit()
        conn.close()
...
async def main():
    ...
    # Запуск polling (без initialize и updaters)
