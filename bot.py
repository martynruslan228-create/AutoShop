...
def generate_summary(data):
    user = data.get('username', 'приховано')
    tg_status = f"@{user}" if data.get('show_tg') == "Так" else "приховано"
...
async def new_ad(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Введіть марку:", reply_markup=ReplyKeyboardRemove()); return MAKE
...
async def get_desc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['description'] = update.message.text; await update.message.reply_text("Фото + /done:", reply_markup=ReplyKeyboardMarkup([["➡️ Без фото"]], resize_keyboard=True)); return PHOTOS
async def get_photos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo: context.user_data['photos'].append(update.message.photo[-1].file_id)
    return PHOTOS
async def done_photos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Тел:", reply_markup=ReplyKeyboardRemove()); return PHONE
async def get_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['phone'] = update.message.text; await update.message.reply_text("Показати TG?", reply_markup=ReplyKeyboardMarkup([["Так", "Ні"]], resize_keyboard=True)); return SHOW_CONTACT
async def get_tg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['show_tg'] = update.message.text; context.user_data['username'] = update.effective_user.username or "user"
    ...
    if update.message.text == "✅ Так":
        ...
        try:
            if not ps: msg = await context.bot.send_message(CHANNEL_ID, cap, parse_mode=ParseMode.HTML)
                ...
                media = [InputMediaPhoto(p, caption
