...
(MAKE, MODEL, YEAR, GEARBOX, FUEL, DRIVE, DISTRICT, TOWN, PRICE, DESCRIPTION, PHOTOS, PHONE, SHOW_CONTACT, CONFIRM) = range(14)
...
async def get_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Показати TG?", reply_markup=ReplyKeyboardMarkup([["Так", "Ні"]], resize_keyboard=True))
    return SHOW_CONTACT
...
async def my_ads(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur.execute('SELECT id, details, msg_id FROM ads WHERE user_id = ? ORDER BY id DESC', (update.effective_user.id,))
    rows = cur.fetchall(); conn.close()
    if not rows: await update.message.reply_text("Немає оголошень."); return
    for r in rows:
        kb = [[InlineKeyboardButton("🗑 Видалити", callback_data=f"del_{r[0]}_{r[2]}")]]; await update.message.reply_text(r[1], parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(kb))
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d[0] == "del":
        ...
        await q.answer("Видалено"); await q.edit_message_text("🗑 Видалено.")
